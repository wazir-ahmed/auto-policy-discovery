...
func GetLastedHTTPPolicy(existingPolicies []types.KnoxNetworkPolicy, policy types.KnoxNetworkPolicy) []types.KnoxNetworkPolicy {
	...
	for _, exist := range existingPolicies {
		...
		if exist.Metadata["namespace"] == policy.Metadata["namespace"] &&
			...
			if existPolicyType == PolicyTypeEgress {
			...
			if strings.Contains(existRule, "matchLabels") {
				// check matchLabels
				matchLabels := true
			...
			} else if strings.Contains(existRule, "toCIDRs") && existPolicyType == PolicyTypeEgress {
				// check CIDRs
				matchCIDRs := true
			...
			} else if strings.Contains(existRule, "toFQDNs") && existPolicyType == PolicyTypeEgress {
				// check FQDNs
				matchFQDNs := true
			...
			} else if strings.Contains(existRule, "toEntities") || strings.Contains(existRule, "fromEntities") {
